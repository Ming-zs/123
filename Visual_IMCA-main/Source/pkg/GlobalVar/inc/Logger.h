...
#define WORD_BLOD_WHITE    BLINK_CODE WORD_WHITE
...
#define LOG_MSG_COLOR           WORD_BLOD_WHITE
#endif
#ifndef LOG_DEBUG_COLOR
#define LOG_DEBUG_COLOR         WORD_GREEN
#endif
...
#define LOG_4(format, ...)      fprintf(LOG_OUT, format, ##__VA_ARGS__)
#else
#define LOG_4(format, ...)      ((void)0)
#endif
#define LOG_5(format, ...)          ((void)0)
...
#ifndef NDEBUG
#define LOGD(format, ...)       LOG(LOG_DEBUG,   STR_CTR(LOG_DEBUG_COLOR,  "<DEBUG>: " format), ##__VA_ARGS__)
#else
#define LOGD(format, ...)       ((void)0)
#endif
#define LOGE_INFO(format, ...)      LOG(LOG_ERROR, \
                                            STR_CTR(LOG_ERROR_COLOR,  "<ERROR>\tat") \
                                            STR_CTR(LOG_LINK_COLOR,  " [ %s:%d ]") \
                                            STR_CTR(LOG_ERROR_COLOR, " : "   format), \
                                            __FILE__, __LINE__, ##__VA_ARGS__)
#define LOGW_INFO(format, ...)      LOG(LOG_WARNING, \
                                            STR_CTR(LOG_WARNING_COLOR,"<WARNING>\tat") \
                                            STR_CTR(LOG_LINK_COLOR,"[%s:%d]") \
                                            STR_CTR(LOG_WARNING_COLOR, " : " format), \
                                            __FILE__, __LINE__, ##__VA_ARGS__)
#define LOGM_INFO(format, ...)      LOG(LOG_MSG, \
                                            STR_CTR(LOG_MSG_COLOR,    "<MSG>\tat") \
                                            STR_CTR(LOG_LINK_COLOR,    "[%s:%d]") \
                                            STR_CTR(LOG_MSG_COLOR, " : "     format), \
                                            __FILE__, __LINE__, ##__VA_ARGS__)
#ifndef NDEBUG
#define LOGD_INFO(format, ...)  LOG(LOG_MSG, \
                                    STR_CTR(LOG_DEBUG_COLOR,    "<DEBUG>\tat") \
                                    STR_CTR(LOG_LINK_COLOR,    "[%s:%d]") \
                                    STR_CTR(LOG_DEBUG_COLOR, " : "     format), \
                                    __FILE__, __LINE__, ##__VA_ARGS__)
#endif
